...
// Client thread that will send the pressed and entered keyboard key without puasing the process
void *client(void *data){
	...
	// Start event loop when connection is made
	while(serverConnectionId){
		if(_keyboardEventListener()){
			...
			if(ch!='\0' && ch!='\n'){
				...
				ic->dataIsReady = 1; // flag that data is ready
			}
...
void *server(void *data){
	...
	while(isLiving){
		// Await data to be ready and served by L1, and L2 respectively
		...
		if(receiveId > 0){
			...
			sd->dataIsReady = 0; // flag that data has been consumed and notify L1 and L2 respectively
